...
export default function Customize() {
  ...
  const cakeSizes = [
    ...
    { label: '2호', cm: '18cm', circleSize: 'w-16 h-16', scale: 'scale-100' },
  ]
  const sheets = [
  ...
  return (
    <div className="w-full mx-auto bg-chat-bg h-screen flex flex-col font-sans text-gray-800 relative overflow-hidden">
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="flex justify-between items-center py-2 px-1 gap-2">
          ...
          <div className="flex items-baseline gap-0.5">
            <span className="text-[1px] font-bold text-sub-gray-100">
            ...
            {uploadedImage ? (
              <div className="relative w-full h-full flex items-center justify-center animate-fadeIn p-4">
                ...
                <button
                  ...
                  className="absolute top-0 right-0 bg-[#FF9980] text-white w-7 h-7 rounded-full shadow-md flex items-center justify-center text-xs font-bold"
                >
                  ✕
                </button>
              </div>
                ...
                {viewMode === 'top' ? (
                          ...
                          <div className="relative w-full h-full">
                            {letteringStyle === 'center' ? (
                              <div className="absolute inset-0 flex items-center justify-center">
                                <div className="px-4 py-1.5 rounded-lg bg-white border border-gray-200 shadow-sm">
                                  <span
                              ...
                              <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
                                ...
                                <g transform="translate(0, -22)">
                                  <use
                                    ...
                                    strokeLinecap="round"
                                  />
                                  <text className="text-[9px] font-bold fill-gray-700">
                                    <textPath
                                      href="#topCurve"
                                      startOffset="50%"
                                      textAnchor="middle"
                                    >
                                      <tspan dy="3.5">{lettering || '생일 축하해 ❤️'}</tspan>
                                    </textPath>
                                  </text>
                                </g>
                  ...
                  <div className="relative w-full h-full flex items-center justify-center animate-fadeIn pt-4 mb-10">
                    <div className="relative w-44 h-36 scale-110">
                      <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 w-52 h-6 bg-chat-header rounded-[3.744px]" />
                      <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-[76.788px] h-6 bg-[#EBCBCB] rounded-[3.744px]" />
                      <div className="absolute -bottom-15 left-1/2 -translate-x-1/2 w-52 h-6 bg-chat-header rounded-[3.744px]" />
                        ...
                        <div className="w-1/2 h-full flex flex-col p-1 gap-1">
                          {[1, 2, 3].map((i) => (
                            <div key={i} className="flex-1 flex flex-col gap-1">
                              <div
                                className="flex-2 rounded-sm transition-colors"
      ...
      >
        ...
        >
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-14 h-14 bg-chat-bg rounded-full flex items-center justify-center transition-all duration-200 active:scale-95">
            <motion.svg
          ...
          <div className="flex-1 overflow-y-auto pb-48 scrollbar-hide">
            {currentTab === '디자인' && (
              <div className="space-y-10 animate-fadeIn">
                <section>
                  <OptionHeader title="사이즈" tag="인기 옵션" value="도시락" />
                    ...
                    {cakeSizes.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative bg-white rounded-2xl p-4 flex flex-col items-center justify-between h-36 border-2 transition-all ${isSelected ? 'border-[#FF9980]' : 'border-transparent'}`}
                        >
                          <div
                          ...
                          <div className="flex-1 flex items-center justify-center mt-4">
                            <div
                              className={`${item.circleSize} rounded-full bg-chat-header flex items-center justify-center`}
                            >
                              <span className="text-[9px] font-bold text-review-default">
                                {item.cm}
                              </span>
                            </div>
                          </div>
                ...
                <section>
                  <OptionHeader title="모양" tag="인기 옵션" value="하트" />
                  <div className="grid grid-cols-3 gap-3">
                    {['원형', '하트', '사각형'].map((shape) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 flex items-center justify-center mt-4 mb-3 text-chat-header">
                            ...
                            {shape === '하트' && (
                              <svg width="56" height="56" viewBox="0 0 72 72" fill="currentColor">
            ...
            {currentTab === '맛' && (
              <div className="space-y-10 animate-fadeIn">
                <section>
                  <OptionHeader title="시트" tag="인기 옵션" value="초코" />
                  <div className="grid grid-cols-3 gap-3">
                    {sheets.map((sheet) => (
                      <button
                        key={sheet.label}
                ...
                <section>
                  <OptionHeader title="크림" tag="인기 옵션" value="크림치즈" />
                  <div className="grid grid-cols-3 gap-3">
                    {creams.map((cream) => (
                      <button
                        key={cream.label}
                        onClick={() => setSelectedCream(cream.label)}
                        className={`bg-white rounded-3xl p-5 flex flex-col items-center border-2 transition-all ${selectedCream === cream.label ? 'border-[#FF9980]' : 'border-transparent'}`}
                      >
                        ...
                        <span className="font-bold text-[15px]">{cream.label}</span>
                      </button>
                    ))}
                  </div>
                </section>
            ...
            {currentTab === '데코' && (
              <div className="space-y-10 animate-fadeIn">
                <section>
                  <OptionHeader title="아이싱" tag="인기 옵션" value={1} />
                ...
                <section>
                  <OptionHeader title="레터링" tag="인기 옵션" value="가운데정렬" />
                  <span className="text-[12px] text-[#57504F]">줄바꿈 그대로 적용됩니다.</span>
                  <textarea
                    value={lettering}
                    onChange={(e) => setLettering(e.target.value)}
                    className="w-full py-2.25 pl-4.25 rounded-[5px] mt-3 bg-white border-none shadow-inner focus:ring-1 focus:ring-[#FF9980] outline-none text-sm resize-none"
                    rows={2}
                  />
                </section>
                <section className="mt-4 px-1">
                  <div className="flex gap-2 mb-4 p-1 rounded-full">
                    {[
                      { id: 'align', label: '정렬' },
                      { id: 'color', label: '색상' },
                    ].map((tab) => {
                      const isActive = decoTab === tab.id
                      return (
                        <button
                          key={tab.id}
                          onClick={() => setDecoTab(tab.id as DecoSubTab)}
                          className={`px-4 py-1 rounded-full text-[13px] font-bold transition-all ${isActive ? 'bg-[#FFE6DF] text-[#333] border border-[#FF9980]' : 'bg-white text-gray-400'}`}
                        >
                          {tab.label}
                        </button>
                      )
                  ...
                  {decoTab === 'align' && (
                    <div className="grid grid-cols-3 gap-2 mt-3">
                      {[
                        ...
                        const isSelected = letteringStyle === style.id
                        return (
                          <button
                            key={style.id}
                            onClick={() => {
                            ...
                            className={`relative flex flex-col items-center justify-between w-full aspect-[1/1.1] p-2 rounded-[18px] border-[1.5px] transition-all ${isSelected ? 'border-[#FF9980] bg-white shadow-sm' : 'border-[#F0F0F0] bg-white'}`}
                          >
                            <div
                              className={`absolute top-2 left-2 w-4.5 h-4.5 rounded-full flex items-center justify-center ${isSelected ? 'bg-[#FF9980]' : 'border border-[#E0E0E0] bg-white'}`}
                            >
                              {isSelected && (
                                <svg
                                  viewBox="0 0 24 24"
                            ...
                            <div className="flex-1 flex items-center justify-center mt-2">
                              <div
                                className={`w-12 h-12 rounded-full flex items-center justify-center ${isSelected ? 'bg-[#FFF2EF]' : 'bg-[#F8F8F8]'}`}
                              >
                                <svg
                                  viewBox="0 0 30 30"
                                  className={`w-8 h-8 ${isSelected ? 'text-[#FF9980]' : 'text-[#D0D0D0]'}`}
                                >
                                  {style.icon}
                                </svg>
                              </div>
                            ...
                            <span
                              className={`text-[11px] font-bold mb-1 ${isSelected ? 'text-gray-800' : 'text-gray-400'}`}
                            >
                              {style.label}
                            </span>
                          </button>
