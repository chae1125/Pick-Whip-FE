...
function toOrderInfo(it: OrderHistoryItem): OrderInfo {
  const isError = it.stepStatus === 'ERROR'
  ...
  return {
    ...
    progressLabel: isError ? (it.rejectReason ?? '제작 불가') : (it.bottomMessage ?? ''),
    progress: resolveProgress(it),
    previewText: '프리뷰 보기',
    chipText: isError ? '제작 불가' : (it.topMessage ?? ''),
    detail: {
...
export default function OrderCompleteListPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        setError(null)
        const result = await getOrderHistory({ type: 'COMPLETE', limit: 10 })
        setItems((result.content ?? []).map(toOrderInfo))
      } catch (e: unknown) {
        const message = e instanceof Error ? e.message : '주문 완료 내역 조회 실패'
        setError(message)
      }
    }
    run()
    ...
    <main className="relative pt-10 container w-full bg-white">
      ...
      <div className="w-full mt-6 flex flex-col">
        ...
        {!error && items.length === 0 && (
          <div className="py-10 text-center text-sm text-gray-400">주문 완료 내역이 없습니다</div>
        )}
